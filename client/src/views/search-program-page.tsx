...
import { 
  GlobeIcon, UserIcon, Search, Heart, 
  ChevronDown, ChevronUp, MapPin, Clock, 
  GraduationCap, Calendar, CheckCircle2, Check
} from "lucide-react";
...
// Реальные опции для фильтров
const FILTER_OPTIONS = {
  discipline: ["IT и Data Science", "Бизнес и Управление", "Инженерия", "Медицина и Здравоохранение", "Гуманитарные науки"],
  location: ["США", "Великобритания", "Европа (ЕС)", "Азия (Сингапур, Гонконг)", "Австралия и Канада"],
  tuitionFee: ["Полное финансирование (Гранты)", "До 5 000 000 ₸", "5 000 000 - 10 000 000 ₸", "10 000 000 - 15 000 000 ₸", "Более 15 000 000 ₸"],
  duration: ["1 год", "1.5 года", "2 года", "Более 2 лет"],
  language: ["Английский", "Испанский", "Немецкий", "Французский", "Китайский"],
  deadlines: ["Открыт прием (Ближайшие 30 дней)", "Ближайшие 3 месяца", "Осень 2026", "Весна 2027"]
};
...
const FILTER_CATEGORIES = [
  ...
  { id: 'tuitionFee', label: 'Стоимость обучения', icon: <span className="font-bold text-gray-500 text-sm">₸</span> },
...
export default function SearchPage() {
  ...
  // Состояния открытия аккордеонов
  ...
  return (
    <div className="min-h-screen bg-[#f4f5f7] font-sans relative">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-white">
          ...
          <nav className="hidden md:flex items-center space-x-8 text-sm font-medium">
            <a href="#universities" className="hover:text-blue-300 transition-colors">Университеты</a>
            <a href="#bolashak" className="hover:text-blue-300 transition-colors">Гранты</a>
            <a href="#mentorship" className="hover:text-blue-300 transition-colors">Услуги</a>
          </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-32 pb-20 relative z-10">
        ...
        <div className="mb-8">
          <div className="text-gray-300 text-sm mb-4">Главная &gt; Программы</div>
          ...
          <div className="flex gap-6 border-b border-gray-300/30 pb-[-1px]">
            {['Programmes', 'Universities', 'Scholarships'].map((tab) => (
              ...
              >
                {tab === 'Programmes' ? 'Программы' : tab === 'Universities' ? 'Университеты' : 'Стипендии'}
                {activeTab === tab && (
                  <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-blue-500 rounded-t-md" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 mt-8">
          ...
          {/* ЛЕВЫЙ САЙДБАР: РЕАЛЬНЫЕ ФИЛЬТРЫ */}
          <aside className="w-full lg:w-[300px] shrink-0">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden sticky top-24">
              
              <div className="p-5 border-b border-gray-100 bg-blue-50/50">
                <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsBolashakOnly(!isBolashakOnly)}>
                  <span className="font-semibold text-blue-900">Грант «Болашак»</span>
                  <div className={`w-10 h-5 rounded-full p-1 transition-colors duration-300 ${isBolashakOnly ? 'bg-blue-600' : 'bg-gray-300'}`}>
                    <motion.div layout className="bg-white w-3 h-3 rounded-full shadow-sm" animate={{ x: isBolashakOnly ? 20 : 0 }} />
                  </div>
                </div>
              </div>

              {/* Аккордеоны с реальными данными */}
                ...
                <div key={category.id} className="border-b border-gray-100 last:border-0">
                  ...
                  >
                    <div className="flex items-center gap-3 text-gray-700 font-medium text-sm">
                      <span className="text-gray-400 w-5 flex justify-center">{category.icon}</span>
                      {category.label}
                    </div>
                    {openFilters[category.id] ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                  </button>
                  <AnimatePresence>
                          ...
                          {FILTER_OPTIONS[category.id].map((option, i) => {
                            ...
                            return (
                              <label key={i} className="flex items-start gap-3 cursor-pointer group">
                                ...
                                <span className={`text-sm transition-colors ${isSelected ? 'text-gray-900 font-medium' : 'text-gray-600 group-hover:text-gray-900'}`}>
                                  {option}
                                </span>
                              </label>
                            );
          ...
          {/* ПРАВАЯ ЧАСТЬ: ПОИСК И СПИСОК (Без изменений) */}
          <div className="flex-1 flex flex-col gap-6">
            ...
            <div className="flex justify-between items-end mt-2">
              <h2 className="text-gray-600 font-medium">
                Найдено программ: <span className="text-gray-900 font-bold">{filteredPrograms.length}</span>
              </h2>
                  ...
                  >
                    <div className="flex flex-col md:flex-row justify-between gap-6">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          <span className="text-xs font-medium bg-gray-100 text-gray-600 px-2.5 py-1 rounded-md">{program.type}</span>
                          <span className="text-xs font-medium bg-gray-100 text-gray-600 px-2.5 py-1 rounded-md">{program.format}</span>
                          <span className="text-xs font-medium bg-gray-100 text-gray-600 px-2.5 py-1 rounded-md">{program.attendance}</span>
                          {program.isBolashak && (
                      ...
                      <div className="flex flex-col items-start md:items-end justify-between min-w-[200px]">
                        <div className="flex justify-between w-full md:w-auto items-center md:items-start gap-4 mb-4 md:mb-0">
                          <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-200">
                            ...
                            <span className="font-bold text-green-600">{program.match}%</span>
                          </div>
                        ...
                        <div className="text-left md:text-right w-full mt-auto">
                          ...
                          <p className="text-sm text-gray-500">/ год • {program.duration}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center">
                      <div className="text-sm text-gray-500 flex items-center gap-1">
                        <span className="text-yellow-400">★</span> {program.rating} ({program.reviews})
                      </div>
                      <a href="#" className="text-blue-600 font-medium text-sm hover:text-blue-700 hover:underline">
